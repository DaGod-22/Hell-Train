...
export class Enemy {
  ...
  update(dt, ctx) {
    ...
    this.animT += dt;
    if (this.animT > 0.18) { this.animT = 0; this.frame = (this.frame + 1) % 2; }
    if (this.flashT > 0) this.flashT -= dt;
    if (this.burnT > 0) {
      this.burnT -= dt;
      this.hp -= this.burnDmg * dt;
      if (this.hp <= 0) { this.alive = false; this.onDeath?.(this); }
    }
    ...
    const spdMul = (this.slowT > 0 ? (1 - this.slow) : 1);
    ...
    if (d < this.radius + player.radius) {
      const t = player.takeDamage(this.dmg * dt);
      if (t > 0) ctx.fx.flash(player.x, player.y, '#ff3a3a', 0.05);
    }
  }
  ...
  takeDamage(amount, opts = {}) {
    ...
    if (opts.family === 'fire' && !this.voidTouched) {
      // burn
      this.burnDmg = Math.max(this.burnDmg || 0, opts.burn || 12);
      this.burnT = Math.max(this.burnT || 0, 2.5);
    }
    if (opts.family === 'ice') {
      this.slow = Math.max(this.slow || 0, opts.slow || 0.5);
      this.slowT = Math.max(this.slowT || 0, opts.slowDur || 2);
      if (opts.frozenHeart) this.shield = (this.shield || 0) + 5;
    }
    this.hp -= dmg;
    this.flashT = 0.08;
    if (this.hp <= 0) {
      ...
      // split behavior
      if (this.splitInto) {
        for (let i = 0; i < (this.splits || 2); i++) {
          const ang = Math.random() * TAU;
          ctx.spawnEnemy(this.splitInto, this.x + Math.cos(ang) * 12, this.y + Math.sin(ang) * 12);
        }
      }
    }
    return dmg;
